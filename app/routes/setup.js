...
export default Route.extend(styleBody, {
    ...
    beforeModel() {
        ...
        // with patronus auth users are authenticated on step 2 so we
        // can't use the session.isAuthenticated shortcut
        // TODO: update with actual config value once implemented
        if (!this.get('config.patronusAuth') && this.get('session.isAuthenticated')) {
